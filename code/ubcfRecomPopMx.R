...
UBCF_POP_MX <- function(data, parameter = NULL){
  ...
  predict <- function(model, newdata, n=10, 
    ...
    print(dim(mnRatings))
    ratings <- pmin((t(r_a_norms)/sum_s_uk) ,(mnRatings))
    print(dim(ratings))
  ...
  new("Recommender", method = "UBCF_POP_MX", dataType = class(data),
      ntrain = nrow(data), model = model, predict = predict)
}


recommenderRegistry$set_entry(
  method="UBCF_POP_MX", dataType = "realRatingMatrix", fun=UBCF_POP_MX,
